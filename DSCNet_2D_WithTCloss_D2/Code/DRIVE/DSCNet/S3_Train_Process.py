...
from S3_DSCNet_pro import DSCNet_pro
from S3_Dataloader import Dataloader
from S3_Loss import TCloss
...
def Train_net(net, args):
    ...
    for epoch in range(args.start_train_epoch, args.n_epochs):
        ...
        if epoch >= args.start_verify_epoch:
            ...
            acc_scores = calculate_accuracy(args.Label_Te_txt, args.save_path)
            acc_mean = np.mean(acc_scores)  # 计算所有图片的准确率平均值
            if dice_mean > dice_m:
                dice_m = dice_mean
                torch.save(
                    net.state_dict(),
        ...
        if 'acc_mean' not in locals():
            acc_mean = 0  # 或者其他合适的默认值
        logger.info("Epoch:[{}/{}]  lr={:.6f}  loss={:.5f}  dice_mean={:.4f} "
            "max_dice={:.4f} acc={:.4f}".format(
                epoch + 1,
                args.n_epochs,
        ...
        epoch_losses.append(loss.item())  # 使用 .item() 确保是数值
        if (epoch+1) >= 200 : 
            epoch_dice_means.append(dice_mean)  # dice_mean 已经是数值
    logger.info("finish training!")

    save_plots(epoch_losses, epoch_dice_means, args)
...
def calculate_accuracy(label_dir, pred_dir):
    label_files = read_file_from_txt(label_dir)
    accuracy_scores = []

    for label_file in label_files:
        file_name = label_file.split('/')[-1]
        prediction_file = join(pred_dir, file_name)

        label_img = sitk.ReadImage(label_file)
        pred_img = sitk.ReadImage(prediction_file)

        label_array = sitk.GetArrayFromImage(label_img).astype(np.float32)
        pred_array = sitk.GetArrayFromImage(pred_img).astype(np.float32)

        label_array = (label_array > 0.5).astype(np.float32)
        pred_array = (pred_array > 0.5).astype(np.float32)

        correct_predictions = np.sum(label_array == pred_array)
...
def Predict_Network(net, args):
    ...
    acc = calculate_accuracy(args.Label_Te_txt, args.save_path)
...
def save_plots(epoch_losses, epoch_dice_means, args):
    # 确保保存图表的目录存在
    ...
    # 绘制dice折线图
    plt.plot(epoch_dice_means, label='Dice Mean')
    plt.title('Dice Mean')
    plt.xlabel('Epoch')
    plt.ylabel('Dice Mean')
    plt.legend()
    dice_plot_path = os.path.join(args.plots_dir, 'dice_mean.png')
    plt.savefig(dice_plot_path)
    print(f"Dice Mean plot saved to {dice_plot_path}")
